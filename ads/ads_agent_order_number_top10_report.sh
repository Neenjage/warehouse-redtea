...
CREATE TABLE if not exists ads.ads_agent_order_number_top10_report
(
    month String,
...
ALTER TABLE ads.ads_agent_order_number_top10_report delete where month = toString(toYYYYMM(toStartOfMonth(toDateTime(concat(toString('$import_time'), ' 00:00:00')))));

INSERT INTO ads.ads_agent_order_number_top10_report
SELECT
...
(
    ...
    (
        SELECT
            ...
            arrayEnumerate(array_order_number) AS rank
        FROM
        (
            SELECT
                dro.agent_name,
