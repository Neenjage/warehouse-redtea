...
CREATE TABLE if not exists ads.ads_Finance_overseas_cost_report
(
    id UInt8,
...
ALTER TABLE ads.ads_Finance_overseas_cost_report delete where order_month = toString(toYYYYMM(toStartOfMonth(toDateTime(concat(toString('$import_time'), ' 00:00:00')))));

INSERT INTO table ads.ads_Finance_overseas_cost_report
select
id,
order_month,
actual_carrier_name as carrier_name,
total_cost1 as total_cost,
value_add_tax,
all_cost
from
(SELECT
  ...
  toDecimal64(sum(dro.total_cost),2) as total_cost1,
  toDecimal64(0,2) as value_add_tax,
  toDecimal64(sum(dro.total_cost) + 0,2) as all_cost
FROM
...
and data_plan_name not like '%国内%'
and carrier_id != 0
and total_cost > 0
group by actual_carrier_name,order_month
union all
select
  1 as id,
  toYYYYMM(addHours(dro.end_time,8)) as order_month,
